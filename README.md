...
| 备份 | 全量导出 `.qpbackup`（当前覆盖 6 张基础表）；恢复前二次确认；损坏文件不破坏原库 |
...
| 模块 | 设计要点 |
| --- | --- |
| 移动端导航重构 | 一级导航收敛为 3 Tab（刷题 / 题库 / 我的），「我的」收纳错题本、收藏、统计、备份、设置；桌面侧边栏分组 |
| 主题系统 | 浅色 / 深色 / 跟随系统三态切换，全组件双主题适配 |
| 沉浸式刷题 | 移动端刷题自动隐藏导航、状态栏沉浸（Edge-to-Edge）；桌面标题栏随主题 |
| 设置页 | 主题、AI 配置（Base URL / API Key / 模型名）、AI 判题/解析开关、激励文案开关、测试连接 |
| AI 判题 | 简答（essay）提交后 AI 判题，含失败降级与手动覆写 |
| AI 错题解析 | 答错后异步生成错题解析，本地缓存，失败静默降级 |
| 轻量激励 | 连续答对轻文案、完成反馈、本地连续天数提示 |

## 技术栈

- **应用外壳**：Tauri 2（Rust）
- **前端**：Vue 3 + TypeScript + Tailwind CSS + Pinia
- **数据库**：SQLite（Rust 侧 `sqlx` 连接池；当前 6 张表：`question_bank` / `question` / `tag` / `question_tag` / `answer_record` / `practice_session`；规划新增 `settings`、`ai_analysis_cache` 及 `answer_record.ai_result` 列）
- **题库文件解析**：PapaParse（CSV）+ SheetJS（Excel .xlsx/.xls），前端解析校验，Rust 侧事务批量写入
- **AI 调用（规划中）**：Rust 侧 reqwest（rustls）调用 OpenAI 兼容接口，Key 不暴露前端
...
| 简答 essay | 无机器判分；当前**手动标记对错**，规划中支持 **AI 判题 + 手动覆写** |

- 判分结果优先级（规划后生效）：**手动覆写 > AI 判题 > 机器判分**。
...
- **接口协议**：OpenAI 兼容 Chat Completions，可配置 **Base URL / API Key / 模型名**（兼容 OpenAI、DeepSeek、通义千问等）
...
- **移动端信息架构（规划）**：一级导航仅 3 项——**刷题（首页）/ 题库 / 我的**（收纳错题本、收藏、统计、备份、设置）；桌面端侧边栏分组展示
- **主题（规划）**：浅色 / 深色 / 跟随系统三态；深色主题适配夜间刷题
- **沉浸式（规划）**：移动端进入刷题自动隐藏导航、状态栏沉浸，全屏只留题目卡
...
- 当前备份覆盖 6 张基础表；**规划后**将扩展设置项、AI 判题结果与解析缓存（届时备份包含 API Key 明文，需妥善保管）
...
| 状态 | 内容 |
| --- | --- |
| ✅ 已完成 | MVP 全流程（题库管理、刷题、判分、错题、收藏、统计、备份）、桌面自定义标题栏、CSV/Excel 导入 |
| ⬜ 待开发 | 移动端导航重构（3 Tab）→ 主题系统 → 沉浸式刷题 → 设置页 → 数据库迁移 → AI 判题/解析 → 轻量激励 |

## 许可

本项目基于 [MIT License](LICENSE) 开源。**默认不含任何网络请求、埋点或上报代码**；规划中的 AI 功能为可选增强，仅在用户主动配置 API Key 并开启对应开关后，向用户指定的接口地址发送答题相关数据。
